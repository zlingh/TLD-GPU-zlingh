...
////////////
  ...
  void NNConfgpu(const cv::Mat& example,std::vector<int>& isin,float& rsconf,float& csconf);
  void NNConfBatchgpu(void **batch, float *ans,int detections,double &f2datacost );
  void addpEx(const cv::Mat &p,float conf,int times);
  void addnEx(const cv::Mat &n,float conf,int times);
  void addpExgpu(const cv::Mat &p);
  void addnExgpu(const cv::Mat &n);
